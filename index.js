...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `<img class="game-img" src=${games[i].img} alt=${games[i].name}>
                              <h5>${games[i].name}</h5>
...
const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length.toLocaleString('en-US')} games. Currently, ${unfundedGames == 1 ? "1 game remains" : unfundedGames.length + " games remain"} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const descriptionStats = document.createElement("p");
descriptionStats.textContent = displayStr;
descriptionContainer.appendChild(descriptionStats);
